...
import type { DataForJob, JobsList, QueueConfig } from '@ioc:Queue'
...
export default class BullManager {
  ...
  public process({ queueName }: { queueName?: string }) {
    ...
    worker.on('failed', async (job, error) => {
      ...
      if (job && (job.attemptsMade === job.opts.attempts || job.finishedOn)) {
        ...
        const jobHandler = this.app.container.make(job.name, [job])
  ...
  public getById<K extends string>(queueName: K, id: string) {
    if (!this.queues.has('default')) {
      return this.logger.info(`Queue [${queueName}] doesn't exist`)
    }

    return this.queues.get(queueName)!.getJob(id)
  }
}
